...
export function registerMcpTools(
  ...
  server.registerTool(
    "list_boards",
    {
      ...
      inputSchema: {
        limit: z.number().int().min(1).max(50).default(20).describe("Max boards to return (1-50, default 20)"),
        offset: z.number().int().min(0).default(0).describe("Number of boards to skip (for pagination, default 0)"),
        sort: z.enum(['default', 'last_modified', 'last_opened', 'last_created', 'alphabetically'])
          .default('last_modified')
          .describe("Sort order (default: last_modified)"),
        query: z.string().max(500).optional().describe("Search boards by name"),
        scope: z.enum(['mine', 'all']).default('mine').describe("'mine' = only my boards (default), 'all' = all accessible boards"),
      },
    },
    async ({ limit, offset, sort, query, scope }) => {
      ...
      const response = await miroClient.getBoardsPage(mergedFilter, limit, offset);
