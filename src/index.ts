...
const noFilter =
  (argv['no-filter'] as boolean) || process.env.MIRO_NO_FILTER === 'true';
const configuredTeamId =
  (argv['team-id'] as string) || process.env.MIRO_TEAM_ID;
...
async function initBoardFilter() {
  ...
  try {
    ...
    console.error(
      `Board filter: showing boards owned by ${tokenContext.user.name} (${tokenContext.user.id})`,
    );
  } catch (err) {
    console.error(
      'Warning: Could not auto-detect user for board filtering. Showing all boards.',
      err,
    );
  }
}
